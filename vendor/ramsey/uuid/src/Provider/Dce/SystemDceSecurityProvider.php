...
/**
 * SystemDceSecurityProvider retrieves the users or group identifiers from the system
 */
class SystemDceSecurityProvider implements DceSecurityProviderInterface
{
    /**
     * @throws DceSecurityException if unable to get a users identifier
     *
     * @inheritDoc
     */
    public function getUid(): IntegerObject
    {
        ...
        if ($uid === '') {
            throw new DceSecurityException(
                'Unable to get a users identifier using the system DCE '
    ...
    /**
     * Returns the users identifier for a users on a Windows system
     *
     * Windows does not have the same concept as an effective POSIX UID for the
     * running script. Instead, each users is uniquely identified by an SID
     ...
     * @link https://www.windows-commandline.com/get-sid-of-user/ Get SID of users
     */
    private function getWindowsUid(): string
    {
        $response = shell_exec('whoami /users /fo csv /nh');
    ...
    /**
     * Returns a group identifier for a users on a Windows system
     *
     ...
     * users running the script. Then, we will get the SID (security identifier)
     ...
     *
     * @link https://www.windows-commandline.com/list-of-user-groups-command-line/ List of users groups command line
     */
    private function getWindowsGid(): string
    {
        $response = shell_exec('net users %username% | findstr /b /i "Local Group Memberships"');
